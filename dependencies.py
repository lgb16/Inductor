...
class MemoryDep(Dep):
    ...
    ########################## WELDER / ASTITCH ###########################
    def decide_loop_order_with_index(self, other):
        """
        Can return None if not able to decide loop orders.
        """
        # assert self.num_vars == other.num_vars
    ...
    ######################### WELDER / ASTITCH #####################################
    def get_free_sym_ranges(self) -> Dict[sympy.Symbol, sympy.Expr]:
        if self.is_indirect():
            return self.ranges
...
class ReadWrites:
    ...
    ############################# WELDER / ASTITCH ##################################
    def write_buffer_names(self, ignore_integer_index=True):
        names: OrderedSet[str] = OrderedSet()
        for dep in self.writes:
            if not isinstance(dep, MemoryDep):
                continue
            if not ignore_integer_index or not isinstance(
                dep.index, (int, sympy.Integer)
